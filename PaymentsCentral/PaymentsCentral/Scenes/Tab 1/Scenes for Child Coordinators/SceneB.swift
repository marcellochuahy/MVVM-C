...
protocol SceneBDelegate: class {
  func doSomething()
}

class SceneB: UIViewController {
  
  weak var coordinator: SceneBDelegate?
...
extension SceneB {
  public class func instantiate(delegate: SceneBDelegate?) -> SceneB {
    let viewController = SceneB()
    viewController.coordinator = delegate
    viewController.title = "Scene B"
    return viewController
  }
}

extension SceneB: ViewCodeProtocol {
